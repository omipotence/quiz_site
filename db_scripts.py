...
def add_questions():
    questions = [
        ...
        ('Каким станет утес если упадет в Красное Море?', 'Мокрым', 'Красным', 'Не изменится', 'Фиолетовым'),
        ...
        ('Когда сетью можно вытянуть воду?', 'Когда вода замерзла', 'Когда нет рыбы', 'Когда уплыла золотая рыбка', 'Когда сеть бросили в сайт с ответами для школьных экзаменов'),
        ('Что больше слона но ничего не весит?', 'Тень слона', 'Воздушный шар', 'Парашют', 'Облако')
    ]
    open()
...
def add_links():
    ...
    answer = input("Добавлять связь (y/n): ")
    while answer != "n":
